...
我们知道，`ModelSerializer` 会自动帮我们创建 关联模型类的字段，但是我们也可以显示指定创建的字段

```python
class AccountSerializer(serializers.ModelSerializer):
    class Meta:
        model = User
        fields = "__all__"  # 默认包含所有字段
        fields = ("username", "age")  # 只包含两个字段 "username" 和 "age"
        exclude = ('password',)   # 除了不包含 password 之外，其他字段都包含
```
...
默认的情况下，是将我们 User 模型中的所有字段都搬过来了
